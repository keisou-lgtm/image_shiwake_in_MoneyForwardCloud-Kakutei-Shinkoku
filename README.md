...
iPhoneで領収書を撮影 → Google Drive に保存 → Claude が読み取り・仕訳登録まで自動化するシステムです。
...
> ⚠️ **証憑アップロードは非対応（2026年3月時点）**
>
> マネーフォワードクラウド確定申告の証憑添付APIは、現在利用している MCP（アルファ版）では提供されていません。
> 仕訳登録後、証憑画像の添付は手動で行ってください。
>
> **手動での添付方法：**
> 1. [マネーフォワードクラウド確定申告](https://invoice.moneyforward.com/) にログイン
> 2. 仕訳帳 → 対象の仕訳を開く
> 3. 「証憑を追加」から画像をアップロード
>
> APIが公開され次第、自動化を予定しています。
...
> 📎 **証憑添付は現在非対応のため手動で行ってください**（詳細は「現在の制限事項」を参照）
...
│       ├── mfcAttachmentService.js # MFC 証憑アップロード（API公開待ち）
│       └── imageService.js         # 画像圧縮（5MB超対応、将来の証憑自動添付に備え実装済み）
├── scripts/
